...
def worker(column, indices, criterions, prev_matched_idxs=None):
    ...
    for start_idx, end_idx in indices:
        ...
        for idx in range(start_idx, end_idx):
            ...
            if (prev_matched_idxs is None) or (
                prev_matched_idxs is not None and idx in prev_matched_idxs
            ):
                matched = True
                ...
                if "and" in criterions:
                    if not all(
                        opt(column[idx], value) for opt, value in criterions["and"]
                    ):
                        matched = False
                # Check OR conditions (at least one condition must be true)
                if "or" in criterions:
                    if not any(
                        opt(column[idx], value) for opt, value in criterions["or"]
                    ):
                        matched = False
                # Handle case for single condition (no need for all() or any())
                if "uni" in criterions:
                    if not all(
                        opt(column[idx], value) for opt, value in criterions["uni"]
                    ):
                        matched = False
                if matched:
...
def parallel_processing(column, all_indices=None, matched_idxs=None, criterions={}):
    """
    ...
    if all_indices is None:
        all_indices = [
            (arr[0], arr[-1] + 1)
            for arr in np.array_split(np.arange(len(column)), num_workers)
        ]
    ...
    worker_indices = [
        all_indices[
            (indices_per_worker * i) : min(
                indices_per_worker * (i + 1), len(all_indices)
            )
        ]
    ...
    results = pool.starmap(
        worker,
        [(column, indices, criterions, matched_idxs) for indices in worker_indices],
    )
