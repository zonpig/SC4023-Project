...
class ResalePriceDataMP:
    ...
    def min_price_year_first(self, year, month, town, log_query=False):
        ...
        area_position_match = parallel_processing(self.columns["floor_area_sqm"].data,
                                                  criterions={"uni": [(operator.ge,80)]})
        ...
        months = [self.columns["month"].data[i]["month"] for i in range(len(self.columns["month"].data))]
        years = [self.columns["month"].data[i]["year"] for i in range(len(self.columns["month"].data))]
        
        # STEP: Get year
        year_position_match = parallel_processing(years,
                                                  matched_idxs=area_position_match,
                                                  criterions={
                                                      "uni": [(operator.eq, year)]
                                                  })
        print("2: ", len(year_position_match))

        month_position_match = parallel_processing(months,
        ...
        # STEP: Get town
        town_position_match = parallel_processing(self.columns["town"].data,
                                                  matched_idxs=month_position_match,
                                                  criterions={
                                                      "uni": [(operator.eq, town)]
                                                  })
