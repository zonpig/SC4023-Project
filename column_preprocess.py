...
class ZoneMapping:
    ...
    def __rearrange__(self, column_store, col_name):
        ...
        for col in columns:
            for unique_col_val in mapped_col_idxs:
                rerranged_column_store[col].extend([column_store.columns[col].data[idx] for idx in mapped_col_idxs[unique_col_val]])
    ...
    def fit(self, column_store, col_name):
        ...
        for zone_map, row_idxs_list in mapped_col_idxs.items():
            ...
            zone_min_val = min(rearranged_column_store["floor_area_sqm"][zone_start_idx:zone_end_idx+1])
            zone_max_val = max(rearranged_column_store["floor_area_sqm"][zone_start_idx:zone_end_idx+1])
