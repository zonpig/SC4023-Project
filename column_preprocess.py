...
class ZoneMapping:
    ...
    def fit(self, column_store, col_name):
        ...
        '''
        latest_zone_start_idx = 0
        mapped_col_idxs, rearranged_column_store = self.__rearrange__(column_store=column_store,
                                                                      col_name=col_name)
        
        zone_mapping = {}
        for zone_map in mapped_col_idxs:
            zone_start_idx = latest_zone_start_idx
            zone_end_idx = zone_start_idx + len(zone_map)
            zone_min_val = min(rearranged_column_store[col_name][zone_start_idx:zone_end_idx+1])
            zone_max_val = max(rearranged_column_store[col_name][zone_start_idx:zone_end_idx+1])
            zone_start_idx += len(zone_map)
